	...
	if($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if($msg == ''){
			...
			if($rowNumber17 > 0){
				...
				$msg = 'Result For The Selected Class is already Computed | <a href="viewJComputedResult.php">Click Here To View Computed Results</a>';
		...
		$sql1 = "SELECT * FROM sstudentsexamsrecordtb WHERE classes = '$classes' AND term = '$term' AND sessions = '$sessions'";
		...
		if($rowNumber1 > 0){
			...
			do{
				...
				$name = $rowValue1['studentname'];
				
				$sqljmaths = "SELECT (( select sum(totalscore) from smaths where studentname = '$name' AND classes = '$classes' AND term = '$term' AND sessions = '$sessions' LIMIT 1) + ( select sum(totalscore) from senglish where studentname = '$name' AND classes = '$classes' AND term = '$term' AND sessions = '$sessions' LIMIT 1)+ ( select sum(totalscore) from sagric where studentname = '$name' AND classes = '$classes' AND term = '$term' AND sessions = '$sessions' LIMIT 1)+ ( select sum(totalscore) from sphysics where studentname = '$name' AND classes = '$classes' AND term = '$term' AND sessions = '$sessions' LIMIT 1)+ ( select sum(totalscore) from schemistry where studentname = '$name' AND classes = '$classes' AND term = '$term' AND sessions = '$sessions' LIMIT 1)+ ( select sum(totalscore) from sbiology where studentname = '$name' AND classes = '$classes' AND term = '$term' AND sessions = '$sessions' LIMIT 1)+ ( select sum(totalscore) from sgeography where studentname = '$name' AND classes = '$classes' AND term = '$term' AND sessions = '$sessions' LIMIT 1)+ ( select sum(totalscore) from scivicedu where studentname = '$name' AND classes = '$classes' AND term = '$term' AND sessions = '$sessions' LIMIT 1)+ ( select sum(totalscore) from scomputer where studentname = '$name' AND classes = '$classes' AND term = '$term' AND sessions = '$sessions' LIMIT 1)+ ( select sum(totalscore) from scrs where studentname = '$name' AND classes = '$classes' AND term = '$term' AND sessions = '$sessions' LIMIT 1)+ ( select sum(totalscore) from sfishery where studentname = '$name' AND classes = '$classes' AND term = '$term' AND sessions = '$sessions' LIMIT 1)+ ( select sum(totalscore) from saccounting where studentname = '$name' AND classes = '$classes' AND term = '$term' AND sessions = '$sessions' LIMIT 1)+ ( select sum(totalscore) from seconomics where studentname = '$name' AND classes = '$classes' AND term = '$term' AND sessions = '$sessions' LIMIT 1)+ ( select sum(totalscore) from sigbo where studentname = '$name' AND classes = '$classes' AND term = '$term' AND sessions = '$sessions' LIMIT 1)+ ( select sum(totalscore) from sliterature where studentname = '$name' AND classes = '$classes' AND term = '$term' AND sessions = '$sessions' LIMIT 1)+ ( select sum(totalscore) from sfoodnut where studentname = '$name' AND classes = '$classes' AND term = '$term' AND sessions = '$sessions' LIMIT 1)+ ( select sum(totalscore) from sgovernment where studentname = '$name' AND classes = '$classes' AND term = '$term' AND sessions = '$sessions' LIMIT 1)+ ( select sum(totalscore) from sanimalhus where studentname = '$name' AND classes = '$classes' AND term = '$term' AND sessions = '$sessions' LIMIT 1)+ ( select sum(totalscore) from smusic where studentname = '$name' AND classes = '$classes' AND term = '$term' AND sessions = '$sessions' LIMIT 1)) as 'total'";
				
				
				$queryjmaths = mysqli_query($link,$sqljmaths) or die(mysqli_error($link));
				$rowValue13 = mysqli_fetch_assoc($queryjmaths);
				
						
					$noofsubjects = $rowValue1['noofsubjects'];
					$avg = $rowValue13['total']/$rowValue1['noofsubjects']; 
					$total = $rowValue13['total'];
					$sn = 0;
					$sessions = $rowValue1['sessions'];
					$classes = $rowValue1['classes'];
					$term = $rowValue1['term'];
					
					mysqli_query($link,"INSERT INTO `scomputedresult` (`sn`, `studentname`, `class`, `term`, `session`,`noofsubjects`, `totalscore`, `average`, `dateadded`) VALUES ('$sn', '$name', '$classes', '$term', '$sessions', '$noofsubjects', '$total', '$avg', NOW())") or die(mysqli_error($link));
				...
				while($rowValue1 = mysqli_fetch_assoc($query1));
					...
					$sqlrank = "SELECT * FROM scomputedresult WHERE class = '$classes' AND term = '$term' AND session = '$sessions' ORDER BY average DESC";
					...
					$grades = array();
						...
						$grades = array_unique($grades);
							foreach($grades as $grade) {
								 $pos = str_repeat(ordinal(($i+1)).' ',$occurrences[$grade]);
								 $sqlrank = "UPDATE scomputedresult SET positions = '$pos' WHERE average = '$grade' ORDER BY average DESC";
								 mysqli_query($link,$sqlrank) or die(mysqli_error($link));	
								 
    							$i += $occurrences[$grade];
								
								 $sqlranks = "SELECT * FROM scomputedresult WHERE positions LIKE '%$pos%'";
					$queryranks = mysqli_query($link,$sqlranks) or die(mysqli_error($link));
					$rowValueranks = mysqli_fetch_assoc($queryranks);
					do{
						//
						//echo $rowValuerank['positions'].'<br>';
						$snn = $rowValueranks['sn'];
						$my_arr = explode(" ", trim($rowValueranks['positions']));
						$newpos = $my_arr[0];
						
						
						$sqlup = "UPDATE scomputedresult SET positions = '$newpos' WHERE sn = '$snn'";
						mysqli_query($link,$sqlup) or die(mysqli_error($link));	
						
						
						
						}
					while($rowValueranks = mysqli_fetch_assoc($queryranks));
						}
				...
				$msg = 'No Student Record For The Period Selected';
		...
		$sqlD = "SELECT * FROM scomputedresult WHERE class = '$classes' AND term = '$term' AND session = '$sessions'";
				$queryD = mysqli_query($link,$sqlD) or die(mysqli_error($link));
				$rowValueD = mysqli_fetch_assoc($queryD);
...
<input type="checkbox" class="js-switch" data-size="small" />
	...
	<div class="content">
		<h2 class="content-heading">Compute Senior Student's  Result</h2>
		...
		<div class="row"> 
			<div class="col-12">
				...
				<!-- Examples -->
				<div class="card">
					<div class="card-header">Select Class To Update</div>
					<div class="card-body">
                        ...
                        <table id="example1" class="table table-striped">
								...
								<tr>
									...
									<td>&nbsp;</td>
									<td colspan="2">&nbsp;</td>
									<td><input type="submit" name="view" id="view" value="Compute Class Result"></td>
								</tr>
								<tr>
								  <td>&nbsp;</td>
								  <td>&nbsp;</td>
								...
								<tr>
								  <td colspan="6" align="center"><table width="100%" border="1">
								    <tr>
								      <td width="27%" align="center"><strong>Students Name</strong></td>
								      <td width="10%" align="center"><strong>Class</strong></td>
								      <td width="11%" align="center"><strong>Term</strong></td>
								      <td width="9%" align="center"><strong>Session</strong></td>
								      <td width="14%" align="center"><strong>#. Of Subjects Offered</strong></td>
								      <td width="12%" align="center"><strong>Total Score</strong></td>
								      <td width="9%" align="center"><strong>Average</strong></td>
								      <td width="8%" align="center"><strong>Position</strong></td>
							        </tr>
                                    
                                    <?php do{?>
                                    
								    <tr>
								      <td align="center"><?php echo $rowValueD['studentname']; ?></td>
								      <td align="center"><?php echo $rowValueD['class']; ?></td>
								      <td align="center"><?php echo $rowValueD['term']; ?></td>
								      <td align="center"><?php echo $rowValueD['session']; ?></td>
								      <td align="center"><?php echo $rowValueD['noofsubjects']; ?></td>
								      <td align="center"><?php echo $rowValueD['totalscore']; ?></td>
								      <td align="center"><?php echo substr($rowValueD['average'],0,5); ?></td>
								      <td align="center"><?php echo $rowValueD['positions']; ?></td>
							        </tr>
                                    
                                    <?php }while($rowValueD = mysqli_fetch_assoc($queryD));?>
