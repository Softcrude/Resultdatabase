...
$pinmsg = '';
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		$cardpin = mysqli_real_escape_string($link,trim($_POST['cardpin']));
		
		$term = mysqli_real_escape_string($link,trim($_GET['term']));
		$sessions = mysqli_real_escape_string($link,trim($_GET['sessions']));
		$class = mysqli_real_escape_string($link,trim($_GET['class']));
		$studentname = mysqli_real_escape_string($link,trim($_GET['studentname']));
		
		if($cardpin  == ''){
			
			$pinmsg = 'Please Insert The Card Pin';
		...
		if($msg == ''){
			...
			if($classSubString  == 'JSS'){
					...
					$sqlJ1 = "SELECT * FROM pinsfortest2020 WHERE pinnumber = '$cardpin' LIMIT 1";
					...
					$rowNumberJ1 = mysqli_num_rows($queryJ1);
						...
						if($rowNumberJ1 == 1){
							...
							$_SESSION['termToDisplay'] = $term;
							$_SESSION['sessionsToDisplay'] = $sessions;
							$_SESSION['classToDisplay'] = $class;
							$_SESSION['studentnameToDisplay'] = $studentname;
							header('Location:JuniorClassResult.php');
				...
				//header('Location:untitled.php');
					...
					$rowNumberJ1 = mysqli_num_rows($queryJ1);
						...
						if($rowNumberJ1 == 1){
							...
							$_SESSION['classToDisplay'] = $class;
							$_SESSION['studentnameToDisplay'] = $studentname;
							header('Location:SeniorClassResult.php');
							...
							<div id="tg-content" class="tg-content">
								<div class="tg-results">
									<div class="tg-pagetitle">
										<h2>Insert Card Pin</h2>
									</div>
									
									<div class="tg-widgetcontent">
										<form class="tg-formtheme tg-formsearchcourse" action="" method="post">
											<fieldset><font color="#990000"><?php echo $pinmsg; ?></font>
												<div class="tg-inputwithicon">
													<span class="tg-select">
														<input type="password" name="cardpin">
													</span>
												</div>
												<button type="submit" class="tg-btn">View Result</button>
											</fieldset>
										</form>
									</div>
